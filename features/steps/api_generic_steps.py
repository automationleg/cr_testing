...
@step('Request is sent to "{uri}" URI')
...
@step(
    'Json response is matching the "{json_schema}" json schema from "{schema_file_name}" file'
)
...
@step(
    'List Field "{field_path}" in response json contains the following integer values'
)
def step_impl(context, field_path):
...
@step(
    'List Field "{field_path}" in response json contains the following fee value pairs'
)
def step_impl(context, field_path):
    expected_values_in_list = [
        [int(row["size"]), float(row["fee"])] for row in context.table
    ]
...
@step(
    'Timestamp field "{field_path}" for order id "{order_id}" equal to date: "{date_string}"'
)
